...
/**
  * 更多的处理Task 运行相关，比如提交Task运行，取消作业运行
  */
...
class TaskManager(
  ...
  : Unit = {
    ...
    if (this.leaderSessionID.isDefined) {
      ...
      /** 周期性的向JobManager进行注册TaskManager */
      triggerTaskManagerRegistration()
    }
  }
...
object TaskManager {
  ...
  : Unit = {
    ...
    /** HA服务 ,standalone,zookeeper factory_class*/
